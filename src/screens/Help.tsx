...
export function Help({ navigate }: { navigate: (to: string, replace?: boolean) => void }) {
  return (
    <div className="pb-12">
      ...
      <Section title="設定推測の見かた">
        <p>
          実戦中の画面にある「設定推測」を開くと、設定1〜6それぞれの確からしさが出ます。
          打った回数と、その機種の理論値を突き合わせて計算しています。
        </p>
        <div className="mt-3">
          <Terms
            items={[
              {
                term: '期待設定',
                body: '設定1〜6の確からしさを平均した数字です。3.5が「何とも言えない」状態で、大きいほど高設定寄りです。',
              },
              {
                term: '設定ごとの確からしさ',
                body: 'それぞれの設定である確率です。合計すると100%になります。',
              },
              {
                term: '何がそう言っているのか',
                body: 'ボーナス・ぶどう・単独REGのそれぞれが、単独ではどのあたりを示しているかです。食い違っていれば、まだ判断できる段階ではありません。',
              },
              {
                term: 'やめ時の目安',
                body: '設定1・2である確率が基準を超えると出ます。基準は設定画面で変えられます。',
              },
            ]}
          />
        </div>
        <Caution>
          「まだ判断材料が乏しく」と出ている間は、数字が高くても低くても意味がありません。
          2000回転くらい回すまでは参考程度に見てください。
        </Caution>
        <p className="mt-3 text-[var(--color-muted)]">
          この計算は「設定1〜6が同じ割合で使われている」という前提です。実際の店は高設定を
          あまり入れないので、出た数字より低めに見ておくのが安全です。
        </p>
      </Section>
      ...
      <Section title="統計の見かた">
        <p>
          ホーム画面や履歴画面の「統計」から開けます。上のボタンで期間を切り替えられます。
        </p>
        <div className="mt-3">
          <Terms
            items={[
              {
                term: '累計収支の推移',
                body: '打った日を横に並べ、収支を足し上げた線です。右端が現在の累計です。',
              },
              {
                term: '月別の収支',
                body: '月ごとの勝ち負けです。上に伸びていればプラス、下ならマイナスです。',
              },
              {
                term: '機種別 / 店舗別 / 曜日別',
                body: 'どの機種・店・曜日が自分にとって良かったかが分かります。台数が少ないうちは、たまたまの結果である点に注意してください。',
              },
              {
                term: 'いちばん動いた台',
                body: '最高と最低の記録です。タップするとその日の詳細を開けます。',
              },
            ]}
          />
        </div>
        <p className="mt-3 text-[var(--color-muted)]">
          実戦中の記録は収支が確定していないため、統計には含まれません。
        </p>
      </Section>
